...
app.use('/webhook', express_1.default.raw({ type: 'application/webhook+json' }));
...
async function handleTrackPublished(event) {
    ...
    if (track.type === 0 && track.source === livekit_server_sdk_1.TrackSource.MICROPHONE) {
...
async function startAIAgent(roomName) {
...
async function startSpeechProcessing(roomName, participantId, trackSid) {
...
async function stopSpeechProcessing(roomName, participantId) {
...
async function generateAIResponse(roomName, participantId) {
